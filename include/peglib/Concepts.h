...
} && requires(C c, const C cc, typename C::iterator it, const typename C::iterator cit,
              const typename C::State s, const typename C::NonTerminalType* rule,
              const typename C::RuleState rs, ExpectedItem ei, std::size_t pos) {
    // Position / state ----------------------------------------------------
